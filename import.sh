...
#脚本入参
hexoSrcDir=/Users/name/blogname/source/_posts
mWebBase=/Users/name/mweb_Path
...
do
	...
	newFileName="$(date -j -f "%Y-%m-%d %H:%M:%S" "${dateStr}" +"%s")0000"
	
	#parse title
	title=$(echo "${file}" | cut -d "." -f1| tr '-' ' ')
	# if [ -z "${title}" ]
	# then
...
#	echo "tags is: ${ary[@]}"
	...
	echo "inserting the read more..."
	gsed -i "15i\ " "${mWebDocsDir}/${newFileName}.md"
	gsed -i "16i <!-- more -->" "${mWebDocsDir}/${newFileName}.md"
	gsed -i "17i\ " "${mWebDocsDir}/${newFileName}.md"
